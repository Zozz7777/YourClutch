...
const customerPaymentSchema = new mongoose.Schema({
  paymentId: {
    ...
    required: true,
    unique: true,
    default: () => `cpay_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  },
  customerId: {
    type: String,
    required: true,
...
customerPaymentSchema.index({ paymentId: 1 });
customerPaymentSchema.index({ customerId: 1, paymentDate: -1 });
customerPaymentSchema.index({ invoiceIds: 1 });
customerPaymentSchema.index({ paymentDate: -1 });
customerPaymentSchema.index({ status: 1 });
customerPaymentSchema.index({ referenceNumber: 1 });
customerPaymentSchema.index({ 'reconciliation.reconciled': 1 });
